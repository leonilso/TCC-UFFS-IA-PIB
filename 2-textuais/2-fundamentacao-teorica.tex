...
A presente sessão busca avaliar os estudos até então construídos que perpassam sobre as temáticas correlatas, desde a evolução dos modelos preditivos, a variável estudada: o \ac{pib}, o uso de modelos preditivos em variáveis macroeconômicas, por fim a avaliação e comparação de modelos preditivos.
...
trouxe a possibilidade de coletar e processar grandes volumes de dados através do que chamamos de sistemas de informação. Que para \citeonline{laudon2022sistemas} são essenciais para alcançar objetivos estratégicos dos negócios. Essas informações são fundamentais para toda decisão, seja ela do setor público ou privado.
...
E nesse contexto temos a predição do PIB, que é algo estudado por diversos pesquisadores, muitos dos quais veremos no decorrer dessa revisão de literatura, mas também de órgãos e instituições públicas e privadas, como é o caso do \ac{ipea} que pontua a importância da predição do PIB "para a definição dos parâmetros para o planejamento fiscal e orçamentário do setor público, bem como para a formação de expectativas no setor privado." \citem[p. 6]{santos2022previsao}.

Dado toda essa evolução, é natural perceber a complexificação desse indicador, desde o século XX com o início da econometria, até os dias atuais com os grandes volumes de dados e a necessidade de decisões fundamentadas, esses fenômenos complexos atuais implicam que métodos tradicionais de predição podem não conseguir descrever a realidade com a mesma precisão da época da qual foram elaborados.
...
Os modelos tradicionais, são modelos simples, geralmente associados a métodos lineares de aprendizado de máquina\footnote{Muitos autores argumentam que o aprendizado de máquina, ou \textit{machine learning} é um parte da grande área da IA, o que de certa fora toda econometria tradicional poderia ser enquadrada como Inteligência Artificial, mas na realidade o que chamamos de IA são modelos não-lineares modernos, mas trataremos disso mais profundamente na subseção \ref{subs:AI_models}} "tem esse nome porque assumem que a função que define $E(Y|X)$ é linear nos preditores $X1, X2,... , Xp$." \citem[p. 14]{vasconcelos2017poder}. \citeonline{rebelo2022interpretabilidade} ainda aponta que os modelos chamados tradicionais possuem uma interpretabilidade natural\footnote{A definição de \citeonline{rebelo2022interpretabilidade} de certa forma é mais precisa, pois modelos não-lineares como árvores de decisão são classificadas como modelos tradicionais pela sua interpretabilidade.}, e não possuem a necessidade de um método de interpretação adicional.
...
O período de estudo de \citeonline{oliveira2024gastos} foi o recorte dos anos de 1994 a 2015, com preços atualizados ao ano de 2015. Esse estudo compreendeu duas regiões brasileiras distintas: Sul e Nordeste. As variáveis coletadas para estas regiões foram: o PIB extraído do \ac{ipea}; e a população do \ac{ibge}. Com ambas em mãos os autores calcularam o PIB \textit{per capita
...
Com esses testes o modelo foi estimado para as regiões Sul e Nordeste, de forma em que o PIB \textit{per capita} como endógeno ao modelo e as variáveis de gastos com infraestrutura como exógenas. Os resultados apontaram que para a região nordeste uma variação de 1\% do gasto com transporte pode ocasionar uma variação de 0.07\% do PIB \textit{per capita}. No caso da energia, com a mesma variação de 1\%, espera-se uma variação de 0.01\% do PIB \textit{per capita}. No caso da região Sul as mesmas variação de 1\% no gasto de transporte e energia espera-se ocasionar uma variação de 0.005\% e 0.007\% respectivamente.
...
Uma dessas condições, é ironicamente a frequência. Nesse sentido, em algumas bases, uma variável é semanal, naquela é mensal, em outra anual, e assim para qualquer regularidade imaginável. Por exemplo, o censo demográfico no Brasil só ocorre a cada 10 anos, os dados entre esses anos são estimativas. Outro exemplo, a taxa SELIC que é definida a cada 45 dias, muito mais frequente que um censo, mas parece ser uma eternidade comparada aos preços das ações que tem frequência praticamente imediata. Nesse sentido um modelo que consegue incorporar o comportamento econômico com diferentes frequências é algo relevante. Nesse contexto, surge o MIDAS.
...
Portanto, o MIDAS se difere dos modelos até agora estudados, tendo em vista que só analisemos modelos autorregressivos.

A responsabilidade sobre o início do desenvolvimento do modelo MIDAS recai justamente sobre os autores da citação anterior, no trabalho de \citeonline{Ghysels2004MIDAS}, com a obra intitulada de "\textit{The MIDAS Touch: Mixed Data Sampling Regression Models}\footnote{O nome faz referência a mitologia atrelada ao toque de Midas, associado a habilidade de transformar tudo em sucesso/riqueza}". O contexto do surgimento do MIDAS é justamente a necessidade de um modelo capaz de delinear as diferentes frequências na realidade. Os autores trazem como exemplo o mercado de ações, analise de risco e também a predição macroeconômica pensando na limitação dos dados, 
...
onde
\begin{itemize}
	\item[a)] $Y_{t+1}$ é a variável dependente no período $t+1$;
...
\begin{itemize}
	...
	\item[d)] $)F_t$ é o vetor dos $r$ fatores comuns latentes;
	\item[e)] $e_t$ representa o vetor dos componentes idiossincráticos\footnote{Não chamamos $e_t$ de vetor de erros, pois no contexto do DFM não se trata só de erro de medição ou ruídos, mas sim a parte da variável que não é explicada por fatores comuns}.
\end{itemize}

Dentre os modelos apresentados, tanto em nível de complexidade computacional, assim como a interpretabilidade dos modelos, não no sentido de seus resultados, mas sim de sua construção o DFM é o modelo mais complexo dentre os modelos tradicionais estudados no presente trabalho, desse modo, e seguindo nossa proposta didática ele é o último modelo tradicional que abordaremos. Inclusive a escolha do DFM ser o último é que também o DFM consegue englobar outros modelos em sua composição, inclusive na subsecção \ref{subs:AI_models} trataremos de um modelo DFM que o autor usou modelos de IA em sua composição, deixando o limiar de tradicionais e de IA mais nebuloso, mas também apresentando um potencial, a combinação de modelos.
...
O poder preditivo de um modelo de IA permite que modelos possuam muitas variáveis, e capturem um comportamento dinâmico e entrelaçado das observações, algo que a simples análise humana e pressupostos lineares não seriam capazes de observar. Por outro lado, a decisão de utilizar ou não uma variável, dos pesos que cada valor possuí e da interpretação após treinamento foge do controle do pesquisador, sendo de certa forma, atribuído ao modelo essa tarefa. Assim fica evidente onde modelos de IA tem maior possibilidade: problemas complexos, com muitas variáveis, diversos fatores dinâmicos, com alto grau de correlação e que a interpretação é menos importante que a capacidade preditiva.
...
Contudo um perceptron por si só, não consegue resolver problemas não-lineares, afinal a combinação de entradas e saídas realizando uma operação linear não pode ter uma resposta para um problema não-linear. Mas a verdadeira solução para problemas não-lineares começa a partir da combinação de dois perceptrons. Intuitivamente, um perceptron é um classificador linear simples, mas combinando os resultados das respostas de dois perceptrons, que são lineares, com um terceiro neurônio combinando essas respostas, consegue-se resolver problemas não-lineares. Nesse contexto surge a ideia de redes.
...
Após a invenção do perceptron havia ainda um espaço grande entre um único neurônio e as redes neurais multicamadas . O perceptron tem uma limitação relevante já citada, só consegue resolver problemas que são linearmente separáveis, isso acontece pela maneira como o aprendizado do modelo de \citeonline{rosenblatt1958perceptron} funciona. As entradas do perceptron eram multiplicadas com o peso e somadas com o viés, passada a uma função sinal ou degrau \footnote{A diferença dessas duas funções está na interpretação do 0, enquanto a função sinal tem a seguinte relação $sgn(x) \{-1\text{ se }x < 0; 0 \text{ se } x = 0; \text{ e } 1 \text{ se } x > 0$. Já a função degrau considera o 0 como um valor positivo $H(x) \{-1 \text{ se } x < 0; \text{ e } 1 se x >= 0$, ambas podem ser usadas no perceptron e ambas tem o mesmo problema que discutiremos no decorrer do texto} que resultava na saída. O aprendizado ocorria da seguinte forma: se a saída fosse igual a saída esperada, nenhum peso era alterado, se a saída fosse diferente da saída esperada os pesos eram modificados, até o modelo se ajustar a saída esperada.

Intuitivamente a limitação é explicita, mas para ficar mais evidente suponhamos o problema clássico das redes neurais, resolver o problema do ou-exclusivo, o XOR\footnote{O XOR parece ser um problemas simples, mas essa operação é fundamental dentro da computação, sendo utilizada em áreas fundamentais, como a criptografia, mascarás de bits, armazenamento digital, construção de processadores, construção de redundância,etc. Calcular o XOR significa perceber o comportamento computacional, mas para além disso o comportamento humano, que não é linear, assim como, o XOR também não é}. A característica do XOR é que quando as entradas binárias são diferentes o resultado é 1, e quando as entradas são iguais o resultado é 0, conforme a tabela verdade: 
...
\begin{figure}[htb]
	\centering
	\captionsetup{justification=centering}
	\caption{Função de erro quadrático e segmentos de retas tangentes representando a derivada em diferentes pontos.}
	\vspace{5pt}
...
Com essa noção de como surgiram os modelos baseados em redes neurais, posteriormente com a evolução das técnicas de \ac{ia} surgem os algoritmos de \textit{deep learning} (aprendizado profundo), esses modelos não são diferentes das redes neurais estudadas no presente trabalho, a principal diferença é a escala. No \textit{deep learning} são muitas camadas, imagine da seguinte forma, com zero camadas ocultas temos uma regressão logística ou um perceptron, com uma camada oculta temos as redes neurais, com duas ou mais temos o que chamamos de \textit{deep neural networks} que são redes neurais com muitas camadas ocultas. Esses algoritmos que rodam as grandes tecnologias de \ac{ia} atualmente, como visão computacional, chatbots, carros autônomos, etc. Contudo também apresentam os mesmos problemas apresentados de uma forma geral, a interpretabilidade, mínimos locais, sobreajuste.

Redes com arquiteturas gigantescas podem resolver problemas complexos, porém não tem a interpretabilidade que modelos simples possuem. Nesse ponto toma se a decisão, por exemplo, em um carro autônomo, importa se a IA decide trocar de pista pela proximidade com o retrovisor ou com a roda de outro carro? Se a resposta for não, não existe um problema, pois o resultado foi alcançado, o carro trocou de pista no momento certo e evitou um acidente, não importa se foi pelo retrovisor, pela roda, ou por qualquer outro fator. Nesse sentido a predição econômica caminha pela mesma estrada, e uma pergunta deve ser feita o processo de como eu cheguei no resultado importa? se a resposta for sim, talvez redes neurais não sejam o modelo mais adequado, mas se for não elas apresentam um grande potencial, conforme veremos a seguir.

A partir desse exposto vimos de uma forma simplificada\footnote{A formulação matemática dos modelos de IA existe, e vai muito além da forma geral do perceptron apresentada na equação \ref{eq:perceptron}, na realidade redes neurais profundas envolvem um conhecimento profundo sobre otimização e álgebra linear, que apesar de extremamente relevantes na compreensão do funcionamento das redes neurais, foge completamente do escopo de uma monografia de economia explicar algebricamente o funcionamento dessas redes. Dessa forma sugerimos novamente a leitura de \citeonline{ekman2021learning} que explica de forma mais detalhada, mais do que poderíamos no presente trabalho, o a lógica matemática das redes neurais.} como redes neurais surgiram e como elas funcionam por baixo dos panos, agora podemos ver as aplicações desses modelos para predição econômica.

Começaremos por \citeonline{vasconcelos2017poder} que utilizou modelos de redes neurais em sua tese, diferente de nosso trabalho o autor realizou sua pesquisa em dados de 41 países de 2001 à 2015 (Segundo o autor em alguns casos a série pode até ser mais extensa devido a disponibilidade), essa escolha se deu principalmente por serem membros da \ac{ocde}, mais alguns países que apesar de não serem membros possuem relevância econômica e/ou tinham a disponibilidade de dados.
...
A partir do exposto de \citeonline{amaral2023previsao} foi possível notar outro tipo de arquitetura para redes neurais, essa com uma adequação relevante para séries históricas, porém ainda assim com as limitações inerentes a esse tipo de modelo, que apesar de terem uma explicabilidade alta dos fatores envolvidos no modelo e uma capacidade de captar relações quase que imperceptíveis nas variáveis econômicas, ainda se faz necessário o aval do técnico economista, implicando que a interpretabilidade de um modelo é algo importante.
...
Até o presente momento vimos diversos modelos, suas histórias e aplicações, desdes os ditos tradicionais, até modelos modernos baseados em IA, nesse sentido abordaremos mais um modelo atual as Florestas Aleatórias, ou mais conhecido como \textit{Random Forest} do inglês. 

A criação do \textit{Random Forest} é atribuída a \citeonline{breiman2001random}. Trata-se da designação de uma "combinação de preditores em forma de árvore, de modo que cada árvore depende dos valores de um vetor aleatório amostrado de forma independente e com a mesma distribuição para todas as árvores da floresta." \citem[p. 1, tradução nossa]{breiman2001random}. De uma forma mais simplificada, o \textit{Random Forest} é uma técnica que consiste em criar vários pequenos modelos com preditores escolhidos de forma aleatória, a resposta advém de uma combinação das respostas de todos os modelos, que no caso de modelos regressivos é dada pela média dos valores.

De uma maneira técnica o \textit{Random Forest} 

\begin{citacao}
	É um classificador que consiste em uma coleção de classificadores em forma de árvore $\{h(\mathbf{x}, \theta_k), k=1, ... \}$, onde os $\{k\}$ são vetores aleatórios independentes e identicamente distribuídos, e cada árvore dá um voto unitário para a classe mais popular na entrada $\mathbf{x}$. \citem[p. 6, tradução nossa]{breiman2001random}
\end{citacao}

A força desse algoritmo é lidar com a grande quantidade de dados e capturar o comportamento de uma maneira que cada variável tem um pedaço da relevância. Nos modelos \textit{Random Forest} do tipo de regressão\footnote{Em classificações binárias no \textit{Random Forest}, existe um sistema de votação, onde a maioria vence, um modelo democrático, se a maior quantidade de árvores forem um tipo de resposta, a resposta do modelo como um todo é o resultado da maioria} o resultado final do modelo será a média do resultado de cada uma das árvores. Essa noção é confirmada por \citeonline{landowska2025gdpgfcfdynamicsglobaleconomies} onde o autor pontua como o algoritmo do  "constrói múltiplas árvores de decisão e agrega seus resultados, o que lhe permite lidar com conjuntos de dados de alta dimensão, reduzir o overfitting e fornecer insights valiosos sobre a importância das variáveis." \citem[p. 5]{landowska2025gdpgfcfdynamicsglobaleconomies}.

\citeonline{lemos2025atas} explica que o \textit{Random Forest} é um modelo baseado árvores de regressão, que são um tipo de árvores de decisão. Nesse tipo de modelo "cada nó representa uma condição de decisão baseada em uma variável preditora, fornecendo um valor numérico, que normalmente é a média dos valores observados nas amostras que atingem aquele nó." \citem[p. 8]{lemos2025atas}. Segundo \citeonline{landowska2025gdpgfcfdynamicsglobaleconomies} o \textit{Random Forest} leva vantagem em comparação a modelos econométricos tradicionais, pois conseguem lidar com relação não lineares e interações complexas entre as variáveis. 

A equação que representa o modelo preditivo do \textit{Random Forest} esta disposta na equação \ref{eq:random_forest} apresentada por \citeonline{vasconcelos2017poder}.

\begin{equation}
	\label{eq:random_forest}
	f(X) = \sum_{m=1}^{M}c_m\dot I(X \in \mathbb{R}_m)
\end{equation}

%oferece vantagens significativas em relação aos modelos econométricos tradicionais, principalmente na sua capacidade de lidar com relações não lineares e interações entre variáveis

O random forest como modelo predito de variáveis macroeconômicas foi utilizado por \citeonline{landowska2025gdpgfcfdynamicsglobaleconomies} no contexto da predição do GDP, por  \citeonline{lemos2025atas} para uma abordagem utilizando as atas do COPOM e por \citeonline{kava2022alem} em sua dissertação como um dos modelos candidatos a predição de variáveis macroeconômicas brasileiras.


Existem centenas de outros modelos que seriam excelentes oportunidades de analises e comparações, porém como tudo na economia os recursos são finitos, e apesar das necessidades humanas ilimitadas, o \textit{Random Forest} será o último modelo a ser abordado no presente trabalho. A partir desse ponto não trataremos mais sobre a literatura produzida como um relato do que foi feito, mas sim nos utilizaremos dessa fundação teórica como alicerces para a construção, aplicação e comparação dos modelos nas sessões futuras. 
